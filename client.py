...
class MCPClient:
    def __init__(self):
        ...
        self.opanai_api_key = "sk-ec050c137d624010b04d19dd64b7bebf" # 调用模型的api_key
        self.base_url = "https://dashscope.aliyuncs.com/compatible-mode/v1" # 调用模型url, 这里以deepseek作演示
        self.model = "qwen-plus-latest" # 调用deepseek-v3模型
        ...
        self.exit_stack = AsyncExitStack() # 为了方便而安全地批量管理和自动清理多个异步上下文资源
    ...
    async def process_query(self, query:str)->str:
        ...
        response = self.client.chat.completions.create(
            model=self.model,
            messages=messages,
            tools=available_tools  # 传入一个tools清单，让模型自动选择合适的工具并返回调用参数
        )
...
async def main():
    if len(sys.argv) < 2:
        print("使用方法是： python client.py mcp_server.py")
        sys.exit(1)
